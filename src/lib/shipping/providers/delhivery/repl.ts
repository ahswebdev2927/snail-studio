import { CreateREPLRequest, CreateShipmentResult } from "../../types";
import { getDelhiveryConfig } from "./config";
import { delhiveryFetch } from "./client";

/**
 * Generates a native REPL (Replacement Exchange) B2C shipment in Delhivery.
 * Endpoint: POST /api/cmu/create.json
 * Uses payment_mode = "REPL", warehouse as pickup location, customer address as exchange destination,
 * and warehouse as return destination for the exchanged item. Generates a single AWB for the exchange journey.
 */
export async function createDelhiveryREPL(
  req: CreateREPLRequest
  ...
  // 1. Prepare Consignee (Customer Exchange Location) Address details
  const fullAddress = [req.customerAddress.addressLine1, req.customerAddress.addressLine2]
    .filter(Boolean)
    .join(", ")
    .trim();

  const pincodeInt = parseInt(req.customerAddress.postalCode.trim(), 10);
  if (isNaN(pincodeInt)) {
    throw new Error(`Invalid pincode '${req.customerAddress.postalCode}' provided for replacement shipment.`);
  }

  // 2. Prepare Return Warehouse (Exchange Destination) details
  ...
  // 3. Construct Delhivery REPL Payload
  const itemDescription = req.replacementItemDesc || "Replacement Press-On Nails Set";

  const shipmentData = {
    name: req.customerAddress.name,
    add: fullAddress,
    pin: req.customerAddress.postalCode.trim(),
    city: req.customerAddress.city,
    state: req.customerAddress.state,
    country: req.customerAddress.country || "India",
    phone: req.customerAddress.phone,
    order: req.requestId,
    payment_mode: "REPL",
  ...
  // 5. Validate Response Status & Extract Single Waybill
  ...
  if (!assignedWaybill && (!isRootSuccess || !isPackageSuccess)) {
    ...
    if (!errorMessage) {
      errorMessage = "Replacement REPL creation rejected by Delhivery.";
    }

    throw new Error(`Delhivery REPL Error: ${errorMessage}`);
  }

  if (!assignedWaybill) {
    throw new Error("Delhivery REPL creation succeeded but no waybill number was returned in response.");
  }
