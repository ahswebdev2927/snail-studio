...
import { processReturnRefund } from "@/services/checkout/order.service";
import { z } from "zod";

const returnRefundSchema = z.object({
  returnRequestId: z.string().optional(),
  reason: z.string().min(3, "Return refund notes must be at least 3 characters long"),
  refundType: z.enum(["full", "custom"]).default("full"),
  refundAmountPaise: z.number().min(1).optional(),
});

// POST /api/admin/orders/[id]/return-refund - Process Return Refund (Admin only)
export async function POST(
  ...
  try {
    ...
    const result = returnRefundSchema.safeParse(body);
    if (!result.success) {
    ...
    const { returnRequestId, reason, refundType, refundAmountPaise } = result.data;

    // Privileged action security audit log check
    ...
    try {
      ...
      // Ignore cache revalidation errors outside Next runtime
    }

    return NextResponse.json(refundResult, { status: 200 });
  } catch (error: any) {
    console.error("POST /api/admin/orders/[id]/return-refund error:", error);
    return NextResponse.json(
      { error: error.message || "Failed to process return refund" },
      { status: 400 }
    );
  }
}
